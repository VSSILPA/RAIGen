...
class BaseAutoencoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.config.d_in, self.config.dict_size)
            )
        )
        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.config.dict_size, self.config.d_in)
            )
        )
    ...
    @staticmethod
    def explained_variance(x: torch.Tensor, x_reconstruct: torch.Tensor) -> torch.Tensor:
        """
        Per-sample explained variance, shape == (batch,)

        Var(x) is computed along the last (embedding) dimension so that
        images, sequence tokens, etc. are treated independently.
        """
        # Squared reconstruction error per sample
        per_token_l2 = (x_reconstruct - x).pow(2).sum(dim=-1)
        # Total variance of the input around its batch mean
        per_token_total_variance = (x - x.mean(0)).pow(2).sum(dim=-1)
        # 1 – Fraction-of-variance-unexplained
        return 1 - per_token_l2 / (per_token_total_variance + 1e-8)
...
class GlobalBatchTopKMatryoshkaSAE(BaseAutoencoder):
    def __init__(self, cfg):
        ...
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(cfg.d_in, total_dict_size)
            )
        )
        
        self.W_dec = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(total_dict_size, cfg.d_in)
            )
        )
    ...
    def get_loss_dict(self, x, x_reconstruct, all_acts, all_acts_topk, x_mean, x_std, intermediate_reconstructs):
        ...
        explained_variance = self.explained_variance(x, x_reconstruct)
        sae_out = self.postprocess_output(x_reconstruct, x_mean, x_std)
        output = {
            "sae_out": sae_out,
